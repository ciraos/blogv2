...
export async function generateMetadata(): Promise<Metadata> {
    return generateBlogMetadata("隐私政策");
}

/** 隐私政策页：内容来自 /public/pages/privacy；未发布返回 404；show_comment 为真则显示评论 */
export default async function Privacy() {
    let page: Awaited<ReturnType<typeof getPublicPageApi>> | null = null;
    try {
        page = await getPublicPageApi("/privacy");
    } catch (err) {
        console.error("[privacy] 获取页面失败:", err);
        page = null;
    }
    ...
    // show_comment 为真才挂评论区：target_path 用页面自身路径（如 /privacy）
